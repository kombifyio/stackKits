# ha-kit StackKit: Research & Architecture Document

> Research findings for the `ha-kit` StackKit. No Kubernetes.
...
In modern-homelab, services are at known IPs in docker-compose files. In ha-kit, services can move between nodes. You need service discovery.
...
In modern-homelab, inter-node traffic goes through tunnel (encrypted). In ha-kit with local-only clusters, nodes communicate directly on LAN. This traffic MUST be encrypted.
...
The ha-kit CUE schema needs to extend modern-homelab with:
...
## 15. Summary: What ha-kit Needs That modern-homelab Doesn't
